...
% Figure 2a - violin plots of differences between PRE, STIM, POST
...
% Figure 2b - topographies of differences between PRE, STIM, POST
...
for iComp = 1:numel(comps)
    ...
    hold on
    topoplot(topo_data.(comp).contrast, chanInfo, 'headrad', 0.5,'plotrad',0.51,'numcontour',4,'maplimits',topo_data.(comp).lims);
    
    if size(topo_data.(comp).chclus,2) > 0
        d = zeros(1, numel(chanInfo));
        d(1,topo_data.(comp).chclus) = 0.0005;
        topoplot(d, chanInfo, 'style', 'blank', 'plotdisk', 'on', 'emarkercolors', {[1 1 1]}, 'headrad', 0.5,'plotrad',0.51,'numcontour',4,'maplimits',topo_data.(comp).lims);
    end
    cb = colorbar();
    ylabel(cb, 'T')
